  ...
  'shura-reveal-and-destroy': (state, source, lane) => {
    ...
    if (!canAddRevealedToSide(state, lane, enemy)) {
      return {
        ...
        log: [
          ...state.log,
          { turn: state.turn, text: `${name} : le côté adverse est plein, impossible d'ajouter une carte.` },
        ],
      };
    }
    ...
    next = {
      ...
      log: [
        ...next.log,
        { turn: state.turn, text: `${name} révèle ${drawnDef.name} (${drawnPower}) du deck adverse.` },
      ],
    };

    if (drawnPower < shuraPower) {
      const result = destroyAtLaneForced(next, lane, enemy, (c) => c.uid === placed.uid, source);
      return {
        ...result.state,
        log: [
          ...result.state.log,
          { turn: state.turn, text: `${drawnDef.name} (${drawnPower}) est inférieur à ${name} (${shuraPower}) — détruit !` },
        ],
      };
    }
